...
- **Sidecar runtime contract** ✓ (2026-05-04) — `design/TAURI_SIDECAR.md` written. Python sidecar side fully implemented (2026-05-04): `GET /api/health`, `POST /api/shutdown`, `SENNI_DATA_ROOT` override + `DATA_ROOT.mkdir()` in `paths.py`, first-run config seed in `on_startup`. Tauri shell side (`src-tauri/`) pending Phase 3.
