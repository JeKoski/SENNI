...
- **Tauri v2 core shell** ✓ (2026-05-04) — `src-tauri/` scaffolded: `Cargo.toml`, `build.rs`, `tauri.conf.json`, `capabilities/default.json`, `src/main.rs`, `src/lib.rs`. Sidecar spawn + health poll + graceful shutdown + tray (Show/Hide + Quit) + `SENNI_SKIP_SIDECAR` dev escape hatch. Placeholder icons in `src-tauri/icons/` generated by `scripts/create_tauri_icons.py`. Deferred: crash monitor (auto-restart), sidecar log capture to Tauri log, auto-updater, CI/CD.
